...
def simulator(Nchr = 20, Ngene = 100, Nevents = 10):
    ...
    def fusion(genome, chr, mixing = 0):
        ...
        while A == B: # Just so the same chromosome isn't selected twice
            B = random.choice(chr)
            if A == B:
                B = random.choice(chr)
        
        if A in chr: chr.remove(A)
        if B in chr: chr.remove(B)
    ...
    def translocation(genome, chr):
        ...
        r = np.random.uniform()
        if r == 99999: # RECIPROCAL TRANSLOCATION
        ...
        else: # NON-RECIPROCAL TRANSLOCATION
